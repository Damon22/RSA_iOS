...
+ (NSData *)encryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef isSign:(BOOL)isSign {
	...
	for(int idx=0; idx<srclen; idx+=src_block_size){
		//ZJLog(@"%d/%d block_size: %d", idx, (int)srclen, (int)block_size);
		...
		if (status != 0) {
			JPLog(@"JPSDK -> SecKeyEncrypt fail. Error Code: %d", (int)status);
			ret = nil;
			break;
		}else{
...
+ (NSString *)rsaSignString:(NSString *)plainString WithPrivateKey:(NSString *)privKey {
    if (!plainString || !privKey) {
        NSLog(@"JPSDK: 签名参数不能为空");
        return nil;
    }
    ...
    return [RSA rsaSignData:plainData WithPrivateKey:keyRef];
}
...
+ (NSString *)rsaSignObj:(id)obj WithPrivateKey:(NSString *)privKey {
    ...
    NSError *error;
    NSData *plainData = [NSJSONSerialization dataWithJSONObject:obj options:NSJSONWritingPrettyPrinted error:&error];
    if (!plainData || error) {
        return nil;
    }
    return [RSA rsaSignData:plainData WithPrivateKey:keyRef];
...
+ (NSString *)rsaSignData:(NSData *)plainData WithPrivateKey:(SecKeyRef)keyRef {
...
+ (BOOL)rsaVerifySignature:(NSString *)signature plainString:(NSString *)plainString WithPublicKey:(NSString *)pubKey {
    ...
    // 要验证的签名
    signature = [RSA safeUrlBase64Decode:signature];
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey {
    ...
    // 直接转换为safeURLBase64类型
	return [RSA safeUrlBase64Encode:ret];
}
...
+ (NSString *)encryptObject:(id)obj privateKey:(NSString *)privKey {
    ...
    NSData *data = [RSA encryptData:jsonData privateKey:privKey];
    NSString *ret = base64_encode_data(data);
    // 直接转换为safeURLBase64类型
    return [RSA safeUrlBase64Encode:ret];
}
...
+ (NSData *)decryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef {
	...
	NSMutableData *ret = [[NSMutableData alloc] init];
	for(int idx=0; idx<srclen; idx+=src_block_size){
		//JPLog(@"JPSDK -> %d/%d block_size: %d", idx, (int)srclen, (int)block_size);
		size_t data_len = srclen - idx;
		if(data_len > src_block_size){
			data_len = src_block_size;
		}
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey {
    // 先处理为非safeUrl类型
    NSString *unsafeStr = [RSA safeUrlBase64Decode:str];
	NSData *data = [[NSData alloc] initWithBase64EncodedString:unsafeStr options:NSDataBase64DecodingIgnoreUnknownCharacters];
...
+ (NSString *)safeUrlBase64Decode:(NSString *)safeUrlbase64Str
{
    ...
    //JPLog(@"JPSDK -> Base64原文：%@", base64Str);
    return base64Str;
}
